...
> 커밋/푸시→PR→리뷰의 일반 워크플로우(커밋/푸시 지시 시 PR 금지, PR 생성 요청 시 CI 완료 후 이어서 리뷰, 리뷰는 항상 코멘트로도 게시)는 전역 규칙 `~/.claude/rules/on-demand/github-cli.md` "PR 작업 워크플로우" 를 따른다. 본 절은 cctg 한정 확인 항목이다.

브랜치 정책 SoT 는 [docs/RELEASING.md](docs/RELEASING.md) (`feature/* → develop → main`).

- **→ `develop` PR 리뷰**: README 등 설명 문서와 `CHANGELOG.md` 가 변경 내용에 맞게 업데이트되었는지 확인한다.
- **`develop` → `main` PR 리뷰**: `VERSION` 파일의 버전이 올바르게 갱신되었는지 확인한다 (release.yml 의 릴리스 트리거).
