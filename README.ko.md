...
> **지원 채널 범위** — CCTG의 상태 디렉터리는 Claude Code의 **channels**(`~/.claude/channels/`) 구조를 따른다. 채널 플러그인은 각자의 전역 봇을 `~/.claude/channels/<채널>/`(`.env`·`access.json`·`approved/`·`inbox/`)에 두며, 프로세스별로는 그 플러그인의 `<CHANNEL>_STATE_DIR` 로 덮어쓸 수 있다. CCTG는 현재 **Telegram 과 Discord** 를 구동하며 나머지는 아직 연동 불가다.
...
| **Discord** | `discord@claude-plugins-official` | `~/.claude/channels/discord/` | `DISCORD_STATE_DIR` | ✅ **지원** — `add --channel discord`; DM 은 기본 페어링, 서버채널은 `--group` 으로 시드 |
...
> **채널 추가** — 각 채널은 `lib/channels.sh` 의 `channel_spec` case 블록(8필드: `plugin`/`statedir_env`/`token_key`/`token_required`/`display`/`id_label`/`id_required`/`seed_policy`)으로 기술된다. 새 채널(예: iMessage) 배선은 국소적 변경이다: `channel_spec` arm 추가 + `IMPLEMENTED_CHANNELS` 등재 → `add`/`status`/`doctor`/자동완성이 자동으로 인식한다(해당 플러그인의 id·접근 규약 검증 후). (bash/zsh 자동완성은 `IMPLEMENTED_CHANNELS` 를 로컬 변수로 미러하므로 그 미러도 함께 갱신한다.)
...
| 플래그 | 의미 |
|---|---|
| `--channel <name>` | 등록할 채널 (`telegram` 또는 `discord`; 기본 `telegram`) |
| `--id <num>` | allowlist용 숫자 채널 사용자 ID (Telegram ID / Discord user snowflake). Telegram 은 비대화형 시 필수, Discord 는 선택 |
...
| `--group <id>[:nomention][:allow=m1,m2]` | (Discord) 서버채널을 `access.json` `groups` 에 사전 시드. 반복 가능. 아래 참조 |
...
#### Discord 봇 (`--channel discord`)

`--channel discord` 로 Discord 채널 봇을 같은 방식으로 등록한다. 토큰은 봇 `.env` 에 `DISCORD_BOT_TOKEN` 으로 저장된다. Discord 의 접근 모델은 Telegram 과 다르다:

- **DM 접근은 기본 페어링** — `--id` 없이 등록하면 시드된 `access.json` 의 `dmPolicy` 가 `"pairing"` 이고 allowlist 는 비어 있다. 첫 DM 시 플러그인이 페어링 코드를 반환하며, 터미널에서 `/discord:access pair <code>` 로 승인한다. `--id <본인 user snowflake>` 를 주면 페어링을 건너뛰고 `dmPolicy: "allowlist"` 로 바로 시드한다.
- **서버채널은 `--group` 으로 시드** — `--group` 하나가 채널 snowflake 하나를 `access.json` `groups` 에 시드한다. 컴파운드 토큰으로 채널별 동작을 설정한다:
  - `--group 846209781206941736` → @멘션 필요, 전체 멤버 허용
  - `--group 846209781206941736:nomention` → @멘션 없이 응답(`requireMention: false`)
  - `--group 846209781206941736:allow=184695080709324800,221773638772129792` → 지정 멤버 snowflake 만 트리거 가능
  - 수식어 조합: `--group <id>:nomention:allow=<m1>,<m2>`
  - 여러 채널은 `--group` 을 반복. 채널·멤버 ID 는 숫자(`^[0-9]+$`)여야 하며, 비숫자 값은 거부되고 봇은 **등록되지 않는다**.

```bash
# Discord 봇, 페어링 DM 접근, 서버채널 2개 사전 시드
...
> 런타임 접근 관리(페어링 승인, 그룹 추가/삭제)는 사용자가 터미널에서 실행하는 `/discord:access` 스킬의 몫이다. `cctg add` 는 초기 `access.json` 시드만 담당한다.
...
> 각 봇 행에는 **채널**(`Telegram` / `Discord`)이 표시된다. `jq` 가 있고 봇에 `access.json` 이 있으면 연결 토폴로지도 함께 보인다 — DM 정책과 시드된 서버채널 수, 예: `채널=Discord (pairing, 그룹 0개)` 또는 `채널=Discord (allowlist, 그룹 2개)`. `jq` 가 없으면 채널 표시명만 보인다.
...
--- 공통 설정(권한 정책) ---
  ...
  deny: 5 개 / allow: 0 개
  (채널 플러그인은 전역 설치 필요, 예: /plugin install <channel>@claude-plugins-official — 대상: telegram discord)
```
