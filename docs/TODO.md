...
> **현황 요약**: 핵심 기능(게이트웨이 구동·신뢰성·헬스 기반 liveness)은 이미 견고하다(완료 이력은 `CHANGELOG.md` 참조). 본 문서의 항목은 대부분 "상주 어시스턴트" 완성도와 확장성을 끌어올리는 **옵트인·비파괴** 작업이다.
...
- [방어 코드 / 견고성 (잔여)](#방어-코드--견고성-잔여)
  - [P3 — 상태 디렉터리 권한(0700) 미강제 (문서·코드 불일치)](#p3--상태-디렉터리-권한0700-미강제-문서코드-불일치)
...
`cc-tg.sh` → `lib/*.sh` 런타임 분리는 완료(env/output/channels/config/util/registry/session/commands 8개 모듈). 잔여는 가장 큰 `lib/commands.sh`(현재 ~950줄)를 도메인별(lifecycle / config / registry-cmd 등)로 더 쪼갤지 여부다.
...
> 게이트웨이 신뢰성의 P1·P2 핵심은 이미 해결됐다(완료 이력은 `CHANGELOG.md` + git 이력이 SoT). 아래는 잔여 낮은-우선순위 항목으로, **단일 사용자 dev 도구 전제**상 실제 위험이 낮아 우선순위가 낮다.

### P3 — 상태 디렉터리 권한(0700) 미강제 (문서·코드 불일치)

`docs/configuration.md`(en/ko)는 봇 상태 디렉터리를 **`0700`** 로 명시("treat it like the rest of the `0700` state dir")하지만, 코드는 `mkdir -p`(umask 기본, 보통 755)로 생성한다 — 문서·코드 불일치. (검토분석 review #6.)

- **위치**: `cc-tg.sh`(`$CHANNELS_DIR`), `lib/commands.sh` `cmd_add`(`$SD/inbox`)·`cmd_config`(예약봇 `$sd`).
- **현황(완화)**: `.env`·`access.json`·`launch.env`·`last-session.log` 는 모두 `600`(`write_atomic`/`write_token_env` mktemp 0600, snapshot chmod 600)이라 **내용 노출은 없다**. 디렉터리 755 는 파일 *존재/이름* 만 타 사용자에 노출. 단일 사용자 dev 도구 전제상 위험 낮음.
- **수정 방향(선택)**: 생성 시 명시 모드 — `mkdir -p "$SD" && chmod 700 "$SD"`(예약봇 `$sd` 동일), 또는 add/config 를 `(umask 077; …)` 로 감싼다. 추가로 `doctor` install-integrity 에 "상태 디렉터리가 group/other 접근 가능" 경고를 `.env` 600 점검과 대칭으로 넣을 수 있다.
- **규모**: 저.
...
`tests/` bats 스위트(현재 **242 테스트**)는 등록·명령·라이프사이클(다중 타겟·실패 가드 포함)·스냅샷 watcher·가드 로직·launch 문자열 wiring·텍스트/JSON status(상태별 정렬·버킷 내부 최근 실행순·last-activity 포함)를 격리 상태 트리 + stateful fake tmux + claude stub 으로 검증한다. 아직 안 덮은 경로:
...
- **tmux-absent 경고 테스트 결정성** (검토분석 nit / `v0.6.0/003` CUT-002) — `tests/misc.bats` 의 tmux 부재 경고 테스트가 호스트 PATH 에 의존해 self-skip 한다. 필요한 실도구(jq/awk/sed)만 심링크한 전용 PATH 로 결정화 가능(현 테스트는 CI 에선 정상).
- **`write_atomic` 실패경로** — residue·registry 보존은 덮였으나, 쓰기 실패(예: 읽기전용 디렉터리) 시 `ERR_ADD_WRITE` + 레지스트리/access.json 불변을 단언하는 테스트는 미작성(선택).
