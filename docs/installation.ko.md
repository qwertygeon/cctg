...
| `--version X.Y.Z` | 특정 릴리스 버전(git 태그 `vX.Y.Z`; `vX.Y.Z` 도 수용)을 설치하고 고정(핀)한다. copy 설치는 `git archive` 로 태그를 추출(레포 작업트리 불변), `--dev` 설치는 `git checkout --detach`(작업트리 dirty 시 거부). |
| `--latest` | 추적 브랜치 최신으로 복귀하고 버전 핀을 해제한다. |
