...
`install.sh` checks dependencies, places `cctg` at `~/.local/bin/cctg`, installs a short alias command `cg` (default; `--no-alias` to skip, `--alias=NAME` to rename), installs shell completions, and adds a managed block to your shell rc for PATH + completions. It is safe to re‑run. To install a specific release instead of the latest, use `--version X.Y.Z` (and switch later with `cctg update --version X.Y.Z` / `--latest` / `--list`). Then open a new terminal (or `source ~/.zshrc`) and verify:
